<?php
// filepath: /workspaces/plugin-library/admin/plugin-library-client-page.php
...
function plugin_library_client_settings_page() {
    ?>
    <div class="wrap">
        <h1>Client Plugin Library Settings</h1>
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr valign="top">
                    ...
                    <td>
                        <div id="plugin-library-servers">
                            <?php
                            $servers = get_option('plugin_library_servers', array());
                            if (!empty($servers)) {
                                foreach ($servers as $index => $server) {
                                    ?>
                                    <div class="plugin-library-server">
                                        <input type="text" name="plugin_library_servers[<?php echo $index; ?>][url]" value="<?php echo esc_attr($server['url']); ?>" placeholder="Server URL" />
                                        <input type="text" name="plugin_library_servers[<?php echo $index; ?>][username]" value="<?php echo esc_attr($server['username']); ?>" placeholder="Username" />
                                        <input type="password" name="plugin_library_servers[<?php echo $index; ?>][password]" value="<?php echo esc_attr($server['password']); ?>" placeholder="Password" />
                                        <button type="button" class="button remove-server">Remove</button>
                                    </div>
                                    <?php
                                }
                            }
                        ...
                        <button type="button" class="button add-server">Add Server</button>
                    </td>
                </tr>
            </table>
            <?php submit_button(); ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelector('.add-server').addEventListener('click', function() {
                var index = document.querySelectorAll('.plugin-library-server').length;
                var serverDiv = document.createElement('div');
                serverDiv.classList.add('plugin-library-server');
                serverDiv.innerHTML = '<input type="text" name="plugin_library_servers[' + index + '][url]" placeholder="Server URL" />' +
                                      '<input type="text" name="plugin_library_servers[' + index + '][username]" placeholder="Username" />' +
                                      '<input type="password" name="plugin_library_servers[' + index + '][password]" placeholder="Password" />' +
                                      '<button type="button" class="button remove-server">Remove</button>';
                document.getElementById('plugin-library-servers').appendChild(serverDiv);
            });

            document.getElementById('plugin-library-servers').addEventListener('click', function(event) {
