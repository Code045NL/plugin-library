...
class Code045_Server_Plugin {
    ...
    public function __construct() {
        ...
        $this->api_key = get_option('code045_server_api_key');
    }
    ...
    public function register_rest_routes() {
        register_rest_route('code045/v1', '/plugins', array(
            'methods' => 'GET',
    ...
    public function verify_api_key($request) {
        $auth_header = $request->get_header('X-code045-API-Key');
    ...
    public static function activate() {
        $api_key = get_option('code045_server_api_key');
        if (!$api_key) {
            $api_key = wp_generate_password(32, false);
            update_option('code045_server_api_key', $api_key);
        }
    }
}
